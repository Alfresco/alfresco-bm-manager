...
public class RestAPITest implements TestConstants
{
    ...
    public void setUp() throws Exception
    {
        ...
        api = new TestRestAPI(dao, testService, logService,
                testRunServicesCache);
    }
    ...
    public void connectToMongoDB() throws UnknownHostException
    {
        ...
        assertTrue(dao.createTest("TESTconnectToMongoDB", "connectToMongoDB",
                "R1", 1));
    ...
    {
        Map<String, Object> map = (HashMap<String, Object>) gson.fromJson(json,
                HashMap.class);
        return map;
    }
     ...
     */
    private void checkTestRunState(String test, String run, TestRunState state,
            boolean expected)
    {
        ...
        if (expected)
        {
            assertEquals("State was incorrect for " + test + "." + run + ". ",
                    state, checkState);
        }
        else
        {
            assertNotSame("State was incorrect for " + test + "." + run + ". ",
                    state, checkState);
        }
        ...
        switch (checkState)
        {
            ...
            case COMPLETED:
                ...
                checkTimeValue(checkJson, FIELD_DURATION, 30001L);      // The range
                // is +- 30s,
                // so we
                // check it's
                // in range
    ...
    public void testScenario01()
    {
        ...
        assertFalse(json,
                json.indexOf("\"default\" : \"NEVER-LEAVES-THE-SERVER\"") >= 0);
        assertTrue(json, json.indexOf(MASK) >= 0);
        assertTrue(
                json,
                json.indexOf("\"description\" : \"The password for the user executing the process.\"") >= 0);
        assertTrue(json, json.indexOf("A test for scenario 01.") >= 0);

        assertEquals("Expect exact match from createTest and getTest", json,
                api.getTest("T01"));

        assertTrue(api.getTests(RELEASE, SCHEMA, 0, 2).contains(
                "\"name\" : \"T01\""));
        ...
        assertTrue(json,
                json.indexOf("\"description\" : \"Scenario 01 - Run 01\"") >= 0);
        assertFalse(json,
                json.indexOf("\"default\" : \"NEVER-LEAVES-THE-SERVER\"") >= 0);
        ...
        assertEquals("Expect exact match from createTestRun and getTestRun",
                json, api.getTestRun("TEST01", "01"));

        assertTrue(api.getTestRuns("TEST01", 0, 2, "").contains(
                "\"name\" : \"01\""));
        ...
        json = api.setTestRunProperty("TEST01", "001", "processCount",
                propSetBean);
    ...
    public void testScenario02()
    {
        ...
        assertTrue(json,
                json.indexOf("\"description\" : \"Scenario 02 - Run 01\"") >= 0);
        assertFalse(json,
                json.indexOf("\"default\" : \"NEVER-LEAVES-THE-SERVER\"") >= 0);
        assertTrue(json, json.indexOf("\"default\" : \"admin\"") >= 0);
        assertTrue(json, json.indexOf("\"default\" : \"" + MASK) >= 0);
    ...
    public void testScenario03()
    {
        ...
        assertTrue("'scheduled' not set",
                json.contains("" + testRunSchedule.getScheduled()));
     ...
     */
    private String createTestRun(String test, String testDescription,
            String run, String runDescription)
    {
        TestDetails testDetails;
        TestRunDetails testRunDetails;
    ...
    {
        String json = createTestRun("T04", "A test for scenario 04.", "01",
                "Scenario 04 - Run 01");
        ...
        // the @Before phase
        org.alfresco.bm.test.Test test = ctx
                .getBean(org.alfresco.bm.test.Test.class);
        ...
        schedule.setScheduled(System.currentTimeMillis() + 20000L);         // It is
        // scheduled
        // for 20s
        // from now
        ...
        assertNotNull(
                "The test should have found the test run as it is now scheduled.",
                testRun);
        assertNull(
                "The test run has not hit scheduled time so it must have started its context.",
                testRun.getCtx());
        ...
        test.forcePing();
        assertNull("Should NOT be a test run context after termination",
                testRun.getCtx());

        // Make sure that the test drops the test run from consideration
        test.forcePing();
        assertNull("Test must drop a completed test run",
                test.getTestRun(testRunId));
    ...
    {
        createTestRun("T05", "A test for scenario 05.", "01",
                "Scenario 05 - Run 01");
        ...
        TestRun testRun = test.getTestRun(testRunId);
        assertNull(
                "The test run should not be instantiated until it has been scheduled.",
                testRun);
        ...
        testRun = test.getTestRun(testRunId);
        assertNotNull(
                "The test run shouldbe instantiated once it has been scheduled.",
                testRun);
        ...
        assertTrue(
                "Test run did not progress to completion: "
                        + api.getTestRunSummary("T05", "01"), completed);
        ...
        assertNull("Should NOT be a test run context after completion",
                testRun.getCtx());
    }
    ...
    {
        createTestRun("T06", "A test for scenario 06.", "01",
                "Scenario 06 - Run 01");
     ...
     */
    private void executeTestRun(String testName, String testDescription,
            String runName, String runDescription) throws Exception
    {
        ...
        propSetBean.setValue(mongoHost);
        propSetBean.setVersion(0);
        api.setTestRunProperty(testName, runName, PROP_MONGO_TEST_HOST,
                propSetBean);
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            // Need to keep checking progress
            test.forcePing();
            String testRunSummaryJson = api
                    .getTestRunSummary(testName, runName);
            Map<String, Object> testRunSummaryMap = fromJson(testRunSummaryJson);
            if (TestRunState.COMPLETED.name().equals(
                    (String) testRunSummaryMap.get(FIELD_STATE)))
            {
                completed = true;
                break;              // This is what we were looking for
            }
        ...
        assertTrue(
                "Test run did not progress to completion: "
                        + api.getTestRunSummary(testName, runName), completed);
    ...
    public synchronized void testScenario07() throws Exception
    {
        ...
        executeTestRun("T07", "A test for scenario 07.", "01",
                "Scenario 07 - Run 01");

        // Get report for test run that does not exist
        try
        {
            ResultsRestAPI resultsAPI = api.getTestRunResultsAPI("T07MISSING",
                    "01");
        ...
        String chartJson = resultsAPI.getTimeSeriesResults(0L, "seconds", 1, 5,
                false);
        assertTrue(chartJson.startsWith("[ { \"time\" : "));
        assertTrue(chartJson.contains("[ { \"time\" : "));
        assertTrue(chartJson
                .contains("000 , \"name\" : \"start\" , \"mean\" : "));
        assertTrue(chartJson
                .endsWith(" , \"fail\" : 0 , \"failPerSec\" : 0.0}]"));
        ...
        xlsxBos.close();
        ByteArrayInputStream xlsxBis = new ByteArrayInputStream(
                xlsxBos.toByteArray());
        XSSFWorkbook xlsxWorkbook = new XSSFWorkbook(xlsxBis);
        xlsxBis.close();
        // Write it to a temp file just for fun
        File xlsxFile = File.createTempFile(UUID.randomUUID().toString(),
                ".xlsx");
        ...
        Assert.assertNotNull("No XLSX description: " + xlsxFile,
                xlsxCoreProperties.getDescription());
        Assert.assertTrue("Title in XLSX must contain test run FQN: "
                + xlsxFile, xlsxCoreProperties.getTitle().contains("T07.01"));
        Assert.assertTrue(
                "Description in XLSX must contain test name and description: "
                        + xlsxFile, xlsxCoreProperties.getDescription()
                        .contains("A test for scenario 07."));
        Assert.assertTrue(
                "Description in XLSX must contain test name and description."
                        + xlsxFile, xlsxCoreProperties.getDescription()
                        .contains("Scenario 07 - Run 01"));
        xlsxWorkbook.close();
    }
    ...
    {
        MongoClient testMongoClient = new MongoClientFactory(
                new MongoClientURI(MONGO_PREFIX + mongoHost), null, null)
                .getObject();
        DB testMongoDB = new MongoDBFactory(testMongoClient, "bm21-data")
                .getObject();
        Set<String> testRunCollections = removeSystemValues(
                testMongoDB.getCollectionNames());
        Assert.assertEquals("Unexpected number of collections in results: "
                + testRunCollections, 0, testRunCollections.size());

        executeTestRun("T08", "A test for scenario 08.", "01",
                "Scenario 08 - Run 01");

        testRunCollections = removeSystemValues(
                testMongoDB.getCollectionNames());
        Assert.assertEquals("Unexpected number of collections in results: "
                + testRunCollections, 5, testRunCollections.size());
        ...
        testRunCollections = removeSystemValues(
                testMongoDB.getCollectionNames());
        Assert.assertEquals("Unexpected number of collections in results: "
                + testRunCollections, 2, testRunCollections.size());
    }
    ...
    public synchronized void testScenario09() throws Exception
    {
        ...
        executeTestRun("T09", "A test for scenario 09.", "01",
                "Scenario 09 - Run 01");
        executeTestRun("T09", "A test for scenario 09.", "02",
                "Scenario 09 - Run 02");

        testRunCollections = removeSystemValues(
                testMongoDB.getCollectionNames());
        Assert.assertEquals("Unexpected number of collections in results: "
                + testRunCollections, 8, testRunCollections.size());
    ...
    {
        // Create the test
        executeTestRun("T10", "A test for scenario 10.", "01",
                "Scenario 10 - Run 01");
        // Now get the registered drivers
        String json = api.getTestDrivers("T10", true);
        assertTrue("Driver not found for test: " + json,
                json.contains("\"expires\" : { \"$date\" : \"20"));
    }
    ...
    public synchronized void testScenario11() throws Exception
    {
        ...
        String[] descriptions = { "Test Field 1", "Test Field 2",
                "Test Field 3", "Test Field 4", "Test Field 5" };
        String[] values = new String[5];

        executeTestRun(test, "A test for scenario 11.", run,
                "Scenario 11 - Run 01");

        // get the extra data report service
        DataReportService reportService = this.testRunServicesCache
                .getDataReportService(test, run);
        ...
        reportService.appendData(driver, test, run, sheet3, fieldNames, values);
        reportService.appendData(driver, test, run, sheet3, fieldNames, values);
        reportService.setDescription(driver, test, run, sheet3, fieldNames,
                descriptions);
        ...
        List<String> descList = reportService.getDescription(null, test, run,
                sheet3);
        assertNotNull(
                "Expected a description row for sheet 3, but can't find any ...",
                descList);
        int count = 0;
        for (String desc : descList)
        {
            assertEquals("Description of sheet 3 mismatch. Expected '"
                    + descriptions[count] + "' found '" + desc + "'.",
                    descriptions[count++], desc);
        }

        // validate number of sheets
        String[] sheetNames = reportService.getSheetNames(null, test, run);
        assertEquals("Expected 3 sheets of extra data, found: "
                + sheetNames.length, 3, sheetNames.length);

        // validate number of entries for each extra data
        int countTestEntries = 0;
        for (int j = 0; j < 3; j++)
        {
            DBCursor dbCursor = reportService.getData(null, test, run,
                    sheetNames[j]);
            while (null != dbCursor && dbCursor.hasNext())
            {
                List<String> valuesFromMongo = reportService
                        .getNextValueRow(dbCursor);
                countTestEntries++;
                assertEquals("Expected 5 columns in extra data, found: "
                        + valuesFromMongo.size(), 5, valuesFromMongo.size());
            }
        }
        assertEquals("Expected 6 lines of extra data ... found: "
                + countTestEntries, 6, countTestEntries);

        // delete test
        api.deleteTest(test, true);

        // validate
        assertNull("Sheet names expected to be empty.",
                reportService.getSheetNames(null, test, run));
